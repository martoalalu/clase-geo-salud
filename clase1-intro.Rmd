...
* **Líneas:** Indica un recorrido (calles, ríos,etc.). Está formado por una secuencia de pares de coordenadas (X,Y) que no delimitan un área.
* **Polígonos:** Indica el área / superficie de un elemento (barrio, área hospitalaria, etc.). Está formado por una secuencia de pares de coordenadas que empiezan y terminan en el mismo punto, generando así un área cerrada, delimitada.
...
Siguiendo con la coyuntura vamos a usar datos de covid a nivel barrio en la Ciudad de Buenos Aires que publica el Gobierno de la Ciudad en su portal de datos abiertos [BA Data](https://data.buenosaires.gob.ar).
...
```{r}
ggplot(barrios) + 
  geom_bar(aes(x= reorder(BARRIO, tasa_covid), weight=tasa_covid, fill=tasa_covid)) +
  coord_flip() 
```
...
Vamos a usar un dataset disponibilizado por el Ministerio de Salud del Gobierno Nacional que contiene la ubicación exacta todos los efectores de salud del país. En este caso usaremos una versión filtrada del mismo con los efectores de la Ciudad de Buenos Aires.

```{r}
#Cargamos los datos
salud_caba <- read.csv("https://raw.githubusercontent.com/martoalalu/clase-geo-salud/clase-2021/data/establecimientos_salud_caba.csv", fileEncoding = 'UTF-8')
...
Con tan solo ver la distribución de los puntos podemos identificar que efectivamente los efectores son de la Ciudad de Buenos Aires. A no desesperarse, podemos agregar el mapa de la Ciudad a modo de referencia.

Al igual que en cualquier gráfico de `ggplot()` podemos combinar capas. Vamos a agregar una capa con los barrios detrás para tener la referencia.
...
Genial! Primera clase y ya pudimos hacer un montón de mapas!

A modo de repaso, algunos conceptos importantes que vimos:
